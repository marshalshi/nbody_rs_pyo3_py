...
pub static BODIES: [Planet;N_BODIES] = [
    // Sun
    Planet {
        x: 0.0, y: 0.0, z: 0.0,
...
// Original code of fn `advance` which used in Benchmarks Game
pub fn advance(bodies: &mut [Planet;N_BODIES], dt: f64, steps: i32) {
    for _ in 0..steps {
...
pub fn energy(bodies: &[Planet;N_BODIES]) -> f64 {
    let mut e = 0.0;
...
pub fn offset_momentum(bodies: &mut [Planet;N_BODIES]) {
    let mut px = 0.0;
    let mut py = 0.0;
    let mut pz = 0.0;
